...
// ============================================================================
// Catalog-driven display shape (server fetches, client renders)
// ============================================================================
...
export interface PlanDisplay {
  ...
  name: string
  price: string        // "$20"
  period: string       // "/month" (subscription) or "" (pack)
  reports: string      // "25 credits/month" or "3 credits"
  subtitle?: string    // e.g. "One-time purchase"
  description?: string // pack: "3 credits, use anytime"
  features: string[]
...
export interface PricingClientProps {
  plans: PlanDisplay[]  // subscription_plan rows
  packs: PlanDisplay[]  // credit_pack rows
}

type PaidPlan = string; // any SKU — runtime gates by catalog
...
export default function PricingClient({ plans, packs }: PricingClientProps) {
  ...
  // SKU sets for checkout gating (derived from catalog, not hardcoded)
  const subscriptionSkus = new Set(plans.map((p) => p.sku));
  // Price map for upgrade/downgrade comparison
  const subscriptionPriceBySku: Record<string, number> = Object.fromEntries(
    plans.map((p) => [p.sku, p.priceUsdCents / 100])
  );
  ...
  function requestCheckout(planKey: PaidPlan) {
    ...
    const acknowledged = subscriptionSkus.has(planKey) ? true : undefined;
          ...
          {packs.map((option, idx) => (
            ...
            >
              <div className="mb-4">
                ...
                {option.subtitle && <p className="text-gray-500 text-xs">{option.subtitle}</p>}
        ...
        <div className="grid md:grid-cols-3 gap-6 items-stretch mb-16">
          {plans.map((plan, idx) => (
            ...
            >
              ...
              {(() => {
                const isCurrentPlan = currentPlan === plan.sku
                const currentPrice = currentPlan ? subscriptionPriceBySku[currentPlan] : undefined
                const thisPrice = subscriptionPriceBySku[plan.sku]
              ...
              },
              {
                ...
                a: (() => {
                  const packSummary = packs
                    .filter((p) => p.active)
                    .map((p) => `${p.name.replace(/\s+Pack$/i, '')}: ${p.reports}`)
                    .join(' / ')
                  return `Credit packs (${packSummary}) are one-time purchases with no commitment. Monthly plans give you a set number of credits per month and access to all 9 report types. If you run reports regularly, a monthly plan is better value.`
                })(),
              },
              {
                q: "Which plan is best for my business?",
                a: (() => {
                  // Feature the highlighted plan if one exists, else the middle plan by price
                  const active = plans.filter((p) => p.active)
                  const featured = active.find((p) => p.highlighted)
                    ?? active.sort((a, b) => a.priceUsdCents - b.priceUsdCents)[Math.floor(active.length / 2)]
                  const planBit = featured
                    ? `the ${featured.name} plan at ${featured.price}${featured.period} covers most businesses at ${featured.reports}.`
                    : `a monthly plan covers most businesses.`
                  return `Start free to see the value. If you need more reports, a credit pack is the easiest next step. If you run reports regularly for SEO, competitive tracking, or client work, ${planBit}`
                })(),
              },
              {
