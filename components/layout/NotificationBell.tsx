...
function NotificationIcon({ type }: { type: string }) {
  if (type === 'email_failed' || type === 'report_failed' || type === 'error_alert' || type === 'dispute_request') {
    return (
...
export function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <svg className={`w-5 h-5 transition-colors ${unreadCount > 0 ? 'text-amber-400' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-zinc-900 border border-gray-700 rounded-xl shadow-2xl shadow-black/40 z-50 overflow-hidden">
          {/* Header */}
